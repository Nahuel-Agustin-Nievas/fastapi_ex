import pytest
from app import schemas, models


@pytest.fixture()
def test_vote_data(test_posts, session, test_user):
    new_vote = models.Vote(post_id=test_posts[3].id, user_id=test_user["id"])
    session.add(new_vote)
    session.commit()
    return new_vote


def test_votes_on_post(authorized_client, test_posts):
    response = authorized_client.post("/vote/", json={
        "post_id": test_posts[0].id,
        "dir": 1
    })

    print(response.json())
    assert response.status_code == 201

def test_vote_twice_post(authorized_client, test_posts, test_vote_data):
    response = authorized_client.post("/vote/", json={
        "post_id": test_posts[3].id,
        "dir": 1
    })

    print(response.json())
    assert response.status_code == 409
  

def test_delete_vote_on_post(authorized_client, test_posts, test_vote_data):
    response = authorized_client.post("/vote/", json={
        "post_id": test_posts[3].id,
        "dir": 0
    })

    print(response.json())
    assert response.status_code == 201


def test_delete_nonexistent_vote(authorized_client, test_posts):
    response = authorized_client.post("/vote/", json={
        "post_id": test_posts[2].id,
        "dir": 0
    })

    print(response.json())
    assert response.status_code == 404


def test_vote_on_nonexistent_post(authorized_client):
    response = authorized_client.post("/vote/", json={
        "post_id": 9999,
        "dir": 1
    })

    print(response.json())
    assert response.status_code == 404


def test_unauthorized_vote(client, test_posts):
    response = client.post("/vote/", json={
        "post_id": test_posts[0].id,
        "dir": 1
    })

    print(response.json())
    assert response.status_code == 401